...
@php
  ...
  if( $errors->tambah->any() ) {
    alert()->error('Gagal', 'Gagal menambahkan pengurus satu baru');
  }
  if( $errors->edit->any() ) {
    alert()->error('Gagal', 'Gagal mengupdate data pengurus satu');
  }
...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">Dashboard Pengurus Satu</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
            <li class="breadcrumb-item active">Data Pengurus Satu</li>
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
      ...
      <div class="row">
        ...
        <div class="col-lg-3 col-6">
          ...
          <div class="small-box bg-warning">
            <div class="inner">
              <h3>{{ $pengurus_satus->count() }}</h3>

              <p>Pengurus Satu Terdaftar</p>
            </div>
            <div class="icon">
      ...
      <!-- /.row -->
          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Daftar Pengurus Satu</h3>
              @can('admin')
                <div class="d-flex justify-content-end">
                  <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal-primary">
                    <i class="material-icons" style="font-size:14px">person_add</i> Tambah Pengurus Satu
                  </button>
                </div>
              @endcan
            </div>
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                @php $i = 1 @endphp
                @foreach($pengurus_satus as $pengurus_satu)
                  <tr>
                    <td>{{ $i++ }}</td>
                    <td>{{ $pengurus_satu->id }}</td>
                    <td>{{ $pengurus_satu->name }}</td>
                    <td>{{ $pengurus_satu->email }}</td>
                    <td>{{ $pengurus_satu->no_telephone }}</td>
                    <td>{{ $pengurus_satu->location }}</td>
                    <td>{{ $pengurus_satu->created_at->translatedFormat('d/m/Y H:i') }}</td>
                      ...
                      <div class="btn-group btn-group-sm">
                        ...
                        @can('admin')
                          <a href="#"
                             ...
                             data-id="{{ $pengurus_satu->id }}" 
                             data-name="{{ $pengurus_satu->name }}" 
                             data-email="{{ $pengurus_satu->email }}" 
                             data-no_telephone="{{ $pengurus_satu->no_telephone }}"
                             data-location="{{ $pengurus_satu->location }}"
                             data-profile_picture="{{ $pengurus_satu->profile_picture }}"
                             class="btn btn-success">
                             <i class="fas fa-edit"></i>
                          </a>
                          <a href="{{ route('delete.user', [ 'user_id' => $pengurus_satu->id ]) }}" class="btn btn-danger"><i class="fas fa-trash"></i></a>
                        @endcan
                      </div>
                    </td>
                  </tr>
            ...
            <div class="modal-dialog">
              <div class="modal-content bg-primary">
                <div class="modal-header">
                  <h4 class="modal-title">Tambah Pengurus Satu</h4>
                ...
                <div class="modal-body">
                      ...
                      <form method="POST" action="{{ route('tambah.user') }}" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="user_role" value="pengurus-satu">
    ...
    <div class="modal fade" id="modal-success">
      <div class="modal-dialog">
        <div class="modal-content bg-success">
          <div class="modal-header">
            <h4 class="modal-title">Update Pengurus Satu</h4>
