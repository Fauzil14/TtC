...
@php
  ...
  if( $errors->tambah->any() ) {
    alert()->error('Gagal', 'Gagal menambahkan bendahara baru');
  }
  if( $errors->edit->any() ) {
    alert()->error('Gagal', 'Gagal mengupdate data bendahara');
  }
      ...
      <div class="row">
        ...
        <div class="col-lg-3 col-6">
          ...
          <div class="small-box bg-warning">
            <div class="inner">
              <h3>{{ $bendaharas->count() }}</h3>

              <p>Bendahara Terdaftar</p>
            </div>
            <div class="icon">
      ...
      <!-- /.row -->
          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Daftar Bendahara</h3>
              @can('admin')
                <div class="d-flex justify-content-end">
                  <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal-primary">
                    <i class="material-icons" style="font-size:14px">person_add</i> Tambah Bendahara
                  </button>
                </div>
              @endcan
            </div>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($bendaharas as $bendahara)
                  <tr>
                    ...
                    <td>{{ $bendahara->no_telephone }}</td>
                    <td>{{ $bendahara->location }}</td>
                    <td>{{ $bendahara->created_at->translatedFormat('d/m/Y H:i') }}</td>
                      ...
                      <div class="btn-group btn-group-sm">
                        ...
                        @can('admin')
                          <a href="#"
                             ...
                             data-name="{{ $bendahara->name }}" 
                             data-email="{{ $bendahara->email }}" 
                             data-no_telephone="{{ $bendahara->no_telephone }}"
                             data-location="{{ $bendahara->location }}"
                             data-profile_picture="{{ $bendahara->profile_picture }}"
                             class="btn btn-success">
                             <i class="fas fa-edit"></i>
                          </a>
                          <a href="{{ route('delete.user', [ 'user_id' => $bendahara->id ]) }}" class="btn btn-danger"><i class="fas fa-trash"></i></a>
                        @endcan
                      </div>
                    </td>
                  </tr>
    ...
    <div class="modal fade" id="modal-success">
      <div class="modal-dialog">
        <div class="modal-content bg-success">
          <div class="modal-header">
            <h4 class="modal-title">Update Bendahara</h4>
